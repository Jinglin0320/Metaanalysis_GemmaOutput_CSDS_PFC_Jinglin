...
GMT_scRNASeq_Stress <- read.csv(
  "/labs/twc/JinglinXiong/BAP_Updated_2025/Raw_Data/GMT_scRNASeq_Stress_Cortex.csv",
  stringsAsFactors = FALSE
)

write.table(
  GMT_scRNASeq_Stress,
  file = "/labs/twc/JinglinXiong/BAP_Updated_2025/Output/GMT_scRNASeq_Stress_Cortex.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
BrainGMT <- gmtPathways("/labs/twc/JinglinXiong/BAP_Updated_2025/Output/GMT_scRNASeq_Stress_Cortex.txt")
...
GSEA_Results_scRNASeq<-fgseaSimple(BrainGMT, DEResults_Log2FC_forGSEA_Ranked, nperm=10000, minSize = 10, maxSize = 1000)
#Warning messages:
...
GSEA_Results_scRNASeq$leadingEdge<-vapply(GSEA_Results_scRNASeq$leadingEdge, paste, collapse= ",", character(1L))

#Write out the results:
write.csv(GSEA_Results_scRNASeq, "/labs/twc/JinglinXiong/BAP_Updated_2025/Output/GSEA_Results_scRNASeq.csv")
